...
class particle():
    ...
    def update_momentum(self):
        self.momentum = self.mass * \
            np.sqrt(sum(self.velocity ** 2))  # Pythagorean Theorem
    ...
    def init_array(self):
        self.array = np.array([self.position[0], self.position[1], self.position[2],
                               self.velocity[0], self.velocity[1], self.velocity[2], self.energy, self.momentum, 0])
...
def collision(particle1, particle2, time):
    ...
    temp1 = v_centre_1
    temp2 = v_centre_2
    v_centre_1 = ((mass1 - mass2) / (mass1 + mass2)) * \
        temp1 + 2 * (mass2 / (mass1 + mass2)) * temp2
    v_centre_2 = 2 * (mass1 / (mass1 + mass2)) * temp1 + \
        ((mass2 - mass1) / (mass1 + mass2)) * temp2
    ...
    if ((particle1.energy + particle2.energy - total_energy) < epsilion) != True:
        print("[!] Energy just increased at time " +
              str(time) + ". This defies physics!!!")
